...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
                ...
                possible = [3:5; ir_distances(3:5)']';
            else
                % Pick two of the left sensors based on ir_distances
                possible = [1:3; ir_distances(1:3)']';
            end
            shortest = sortrows(possible, 2);
            shortest = shortest(1:2,1);
            p_1 = ir_distances_wf(:, min(shortest));
            p_2 = ir_distances_wf(:, max(shortest));
